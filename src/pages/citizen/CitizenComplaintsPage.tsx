...
const CitizenComplaintsPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ title: t('complaints.submitError'), variant: 'destructive' });
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Dialog open={dialogOpen} onOpenChange={(o) => { setDialogOpen(o); if (!o) { setDescription(''); clearErrors(); } }}>
            <DialogTrigger asChild>
              <Button className="gap-2"><Plus className="h-4 w-4" />{t('complaints.submit')}</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader><DialogTitle>{t('complaints.submit')}</DialogTitle></DialogHeader>
              <div className="space-y-4">
                <FormFieldWrapper label={t('complaints.description')} error={errors.description} required>
                  <Textarea value={description} onChange={e => { setDescription(e.target.value); clearFieldError('description'); }} rows={4} className={errors.description ? 'border-destructive' : ''} />
                </FormFieldWrapper>
