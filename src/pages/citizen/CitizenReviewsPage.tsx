...
const CitizenReviewsPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ title: t('reviews.submitError'), variant: 'destructive' });
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Dialog open={dialogOpen} onOpenChange={(o) => { setDialogOpen(o); if (!o) { setContent(''); clearErrors(); } }}>
            <DialogTrigger asChild>
              <Button className="gap-2"><Plus className="h-4 w-4" />{t('reviews.submit')}</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader><DialogTitle>{t('reviews.submit')}</DialogTitle></DialogHeader>
              <div className="space-y-4">
                <FormFieldWrapper label={t('reviews.content')} error={errors.content} required>
                  <Textarea value={content} onChange={e => { setContent(e.target.value); clearFieldError('content'); }} rows={4} className={errors.content ? 'border-destructive' : ''} />
                </FormFieldWrapper>
