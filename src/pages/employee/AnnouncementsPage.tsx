  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editId) {
        ...
        toast({ title: t('announcements.updated'), variant: 'success' as any });
      } else {
        await announcementsApi.create(form);
        toast({ title: t('announcements.created'), variant: 'success' as any });
      }
      setDialogOpen(false);
      setEditId(null);
      ...
      toast({ title: t('announcements.createError'), variant: 'destructive' });
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Dialog open={dialogOpen} onOpenChange={(o) => { setDialogOpen(o); if (!o) { setEditId(null); setForm({ title: '', content: '', language: 'EN' }); clearErrors(); } }}>
            <DialogTrigger asChild>
              <Button className="gap-2"><Plus className="h-4 w-4" />{t('announcements.create')}</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader><DialogTitle>{editId ? t('announcements.edit') : t('announcements.create')}</DialogTitle></DialogHeader>
              <div className="space-y-4">
                <FormFieldWrapper label={t('announcements.titleField')} error={errors.title} required>
                  <Input value={form.title} onChange={e => updateField('title', e.target.value)} className={errors.title ? 'border-destructive' : ''} />
                </FormFieldWrapper>
                <FormFieldWrapper label={t('announcements.content')} error={errors.content} required>
                  <Textarea value={form.content} onChange={e => updateField('content', e.target.value)} rows={4} className={errors.content ? 'border-destructive' : ''} />
                </FormFieldWrapper>
                ...
                <Button onClick={handleSubmit} className="w-full" disabled={saving}>
                  {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : editId ? t('common.update') : t('common.create')}
                </Button>
              </div>
            </DialogContent>
          </Dialog>
