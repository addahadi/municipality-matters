...
export const propertySchema = z.object({
  title: z.string().trim().min(2, 'validation.required').max(200, 'validation.tooLong'),
  location: z.string().trim().min(2, 'validation.required').max(300, 'validation.tooLong'),
  superficie: z.string().refine(v => !isNaN(Number(v)) && Number(v) > 0, 'validation.positiveNumber'),
  status: z.enum(['AVAILABLE', 'RENTED', 'AUCTION', 'CLOSED']),
  cahierPrice: z.string().refine(v => v === '' || (!isNaN(Number(v)) && Number(v) >= 0), 'validation.positiveNumber'),
  startingAuctionPrice: z.string().refine(v => v === '' || (!isNaN(Number(v)) && Number(v) >= 0), 'validation.positiveNumber'),
});
...
export const auctionSchema = z.object({
  ...
  startingPrice: z.string().refine(v => !isNaN(Number(v)) && Number(v) > 0, 'validation.positiveNumber'),
});

export const requestSchema = z.object({
  type: z.string().trim().min(2, 'validation.required').max(100, 'validation.tooLong'),
  description: z.string().trim().min(10, 'validation.descriptionMin').max(1000, 'validation.tooLong'),
});
...
export const reviewSchema = z.object({
  content: z.string().trim().min(10, 'validation.descriptionMin').max(1000, 'validation.tooLong'),
});

export const announcementSchema = z.object({
  title: z.string().trim().min(2, 'validation.required').max(200, 'validation.tooLong'),
  content: z.string().trim().min(10, 'validation.descriptionMin').max(2000, 'validation.tooLong'),
  language: z.enum(['EN', 'AR']),
});

export const messageSchema = z.object({
  receiverId: z.string().min(1, 'validation.required'),
  content: z.string().trim().min(1, 'validation.required').max(2000, 'validation.tooLong'),
});

export const paymentSchema = z.object({
  amount: z.string().refine(v => !isNaN(Number(v)) && Number(v) > 0, 'validation.positiveNumber'),
});
